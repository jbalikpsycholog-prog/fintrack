...
COLUMN_ALIASES = {
    ...
    "mena": ["Mena", "mena", "Mena uctu", "currency"],
...
def parse_date(value: str) -> Optional[str]:
    ...
    for fmt in ("%d.%m.%Y", "%Y-%m-%d", "%d/%m/%Y", "%m/%d/%Y", "%d. %m. %Y"):
        try:
            return __import__('datetime').datetime.strptime(value, fmt).strftime("%Y-%m-%d")
        except ValueError:
            continue
    return value
...
def parse_cs_csv(raw_bytes: bytes) -> List[Dict]:
    ...
    for delimiter in (";", ",", "\t"):
        try:
            reader = csv.reader(__import__('io').StringIO(text), delimiter=delimiter)
            rows = list(reader)
